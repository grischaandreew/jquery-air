...
;jQuery.air || (function( j ) {

j.fn.air = j.air = {
	modules: ['bytes','bitmap','matrix','clipboard','drag','drop','file','keyboard','security','request'],
	swfs: ['as3corelib','as3base64'],

	version: "0.0.1",
	log: function ( ) {
		if( air.Introspector.Console )
			if( arguments.length == 0 )
				air.Introspector.Console.log( arguments[ 0 ] );
			else
				air.Introspector.Console.log( arguments );
	},
	load: function ( module, test ) {
		if( module == true || module == false || module == undefined ) {
			$.each( j.air.modules, function( i, n ) {
				j.air.script('../'+n+'.js');
				if( module == true )
					j.air.script('./modules_test/'+n+'.js');
			} );
			$.each(j.air.swfs, function(i,n){
				j.air.swf('../'+n+'.swf');
			});
		} else {
			$.each(j.air.modules, function(i,n){
	...
	swf: function ( path ) {
		document.write( unescape('%3Cscript type="application/x-shockwave-flash" src="'+path+'"%3E%3C/script%3E') );
	},
	type: function ( o ) {
		return String(typeof o).toLowerCase();
	}
};
